...
describe("AuxiliaryData CML Compatibility", () => {
  ...
  it("validates auxiliary data with native scripts - Conway format compatibility", () => {
    ...
    const evolutionNativeScript = {
      type: "sig" as const,
      keyHash: Array.from(keyHashBytes).map(b => b.toString(16).padStart(2, '0')).join('')
    }
    
    const cmlNativeScript = CML.NativeScript.new_script_pubkey(
      CML.Ed25519KeyHash.from_raw_bytes(keyHashBytes)
    )
  ...
  it("validates auxiliary data with multiple native scripts - Conway format compatibility", () => {
    ...
    const evolutionScript1 = {
      type: "sig" as const,
      keyHash: Array.from(keyHash1).map(b => b.toString(16).padStart(2, '0')).join('')
    }
    
    const evolutionScript2 = {
      type: "sig" as const,
      keyHash: Array.from(keyHash2).map(b => b.toString(16).padStart(2, '0')).join('')
    }
    
    const cmlScript1 = CML.NativeScript.new_script_pubkey(
      CML.Ed25519KeyHash.from_raw_bytes(keyHash1)
    )
    
    const cmlScript2 = CML.NativeScript.new_script_pubkey(
      CML.Ed25519KeyHash.from_raw_bytes(keyHash2)
    )
